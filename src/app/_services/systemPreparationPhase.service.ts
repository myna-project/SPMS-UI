...
export class SystemPreparationPhaseService {
  ...
  createSystemPreparationPhase(id: number | string, systemPreparationPhase: SystemPreparationPhase): Observable<any> {
    return this.http.post<SystemPreparationPhase>(environment.apiEndPoint + this.httpUtils.getAdminUrl() + this.poResource + '/' + id + '/' + this.phaseResource, systemPreparationPhase).pipe(
      catchError((err) => { return throwError(err); })
    );
  }

  updateSystemPreparationPhase(id: number | string, systemPreparationPhase: SystemPreparationPhase): Observable<any> {
    return this.http.put(environment.apiEndPoint + this.httpUtils.getAdminUrl() + this.poResource + '/' + id + '/' + this.phaseResource + '/' + systemPreparationPhase.id, systemPreparationPhase).pipe(
      catchError((err) => { return throwError(err); })
    );
  }
  ...
  deleteSystemPreparationPhase(id: number | string, systemPreparationPhase: SystemPreparationPhase): Observable<SystemPreparationPhase> {
    return this.http.delete<SystemPreparationPhase>(environment.apiEndPoint + this.httpUtils.getAdminUrl() + this.poResource + '/' + id + '/' + this.phaseResource + '/' + systemPreparationPhase.id).pipe(
      catchError((err) => { return throwError(err); })
    );
  }
}
